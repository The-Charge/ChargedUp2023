...
public class MagicArm extends SubsystemBase {
  ...
  public double[] getAngles(double _x, double _y) {
    ...
    if (thirdSide + ArmConstants.elbowL < ArmConstants.shoulderL ||
      ...
      // Formating variables
      double firstLOCAdd = thirdSide2 + shoulder2 - elbow2;
      double firstLOCAcos = (firstLOCAdd) / 2 / ArmConstants.shoulderL / thirdSide;

      double secondLOCAdd = shoulder2 + elbow2 - thirdSide2;
      double secondLOCAcos = (secondLOCAdd) / 2 / ArmConstants.elbowL / ArmConstants.shoulderL;

      // Law of cosine
      double oppositeElbowAngle = Math.acos(firstLOCAcos);

      // 2nd application of Law of Cosine
      angles[1] = Math.acos(secondLOCAcos);

      // Aditional formating variables
      double convertAngleAtan = _y / Math.abs(_x);
      double convertAngleAbs = Math.PI / 2 - oppositeElbowAngle - convertAngleAtan;

      // Convert angle from +x axis to the shoulder motor angle
      angles[0] = Math.abs(convertAngleAbs);
       ...
       * Robot design, 1 = acceptable solution
       */
