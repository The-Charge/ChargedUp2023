...
public class MagicArm extends SubsystemBase {
  ...
  public MagicArm() {
    ...
    //Need the code from Mr. Curry to set relative sensor values from abosulte sensor values
  ...
  /**
   * Determine whether the (_x,_y) cooridnate is allowed by the game rule and will not damage the robot.
   * @param _x
   * @param _y
   * @return true if the cooridnate is allowed, false otherwise.
   */
  public boolean isXYInLimit(double _x, double _y){
    if(_y < -ArmConstants.shoulderHeight + 0.1) return false;              //Not into the floor
    if(_y + ArmConstants.shoulderHeight > robotLimit.height) return false; //Not above the height limit
    if(Math.abs(_x) > robotLimit.widthFromCenter) return false;            //Not extend >48 inch from the robot
    if(Math.abs(_x) < robotLimit.robotLength / 2 && _y < 0)return false;    //Not below the shoulder mount while inside the robot
    return true;
  }
  ...
  public double getLigitY(double _x, double _y){
    if(Math.abs(_x) < robotLimit.robotLength / 2) {
      return MathUtil.clamp(_y, 0, ArmConstants.shoulderL - (Math.sqrt(ArmConstants.elbowL * ArmConstants.elbowL - _x*_x)));
    }
    else return MathUtil.clamp(_y, -ArmConstants.shoulderHeight + 0.1, robotLimit.height);
  }

  /**
   * Get the (x,y) coordinate of the current arm tip.
   * @return a double array, the first element is the x value and the second element is the y value.
   */
  public double[] getXY(){
  ...
  /**
   * Get the (x,y) coordinate of the arm tip given a shoulder angle and an elbow angle.
  ...
  /**
   * Determine whether a shoulder and elbow angle pair is allowed by the game rules and won't damage the robot 
   *  and also the shoulder angle and the elbow angle is within the range of the robot design.
   * @param _shoulderAngle
   * @param _elbowAngle
   * @return true if the cooridnate is allowed, false otherwise.
   */
  ...
  /**
   * Calculate desired shoulder and elbow angles to put the arm tip at the (_x,_y) coordinate
   * It uses the triangle formed by the shoulder axle, the elbow axle, and (_x,_y) as the three vertices and use the law of cosine to solve for the angles.
   * @param _x
   * @param _y
   * @return a double array with the first element is the shoulder angle and the second element is the elbow angle and 
   *  the third element is 1 if a solution can be found and -1 for no feasible solution.
   */
  public double[] getAngles(double _x, double _y){
    ...
    double thirdSide = Math.sqrt(thirdSide2);           //Find the length of the segment connecting (_x,_y) point to the shoulder axle
    if (thirdSide + ArmConstants.elbowL < ArmConstants.shoulderL  || 
      thirdSide > ArmConstants.elbowL + ArmConstants.shoulderL) angles[2] = -1;   //No triangle exists
    else{
      double shoulder2 = ArmConstants.shoulderL*ArmConstants.shoulderL;
      double elbow2 = ArmConstants.elbowL*ArmConstants.elbowL;
      double oppositeElbowAngle = Math.acos((thirdSide2 + shoulder2 - elbow2) / 2 / ArmConstants.shoulderL / thirdSide); //Law of cosine
      angles[1] = Math.acos((shoulder2 + elbow2 - thirdSide2) / 2 / ArmConstants.elbowL / ArmConstants.shoulderL); //2nd application of Law of cosine
   ...
   * The starting position needs to be close to a neutral position: shoulder up, elbow down.
   * @param _x
   * @param _y
  ...
  public boolean moveTowardXYFromNeutral(double _x, double _y){
    if (isXYInLimit(_x, _y)){
      ...
      if(angles[2] > 0){
        if (Math.abs(angles[1]-elbowAngl) < 0.05)run(angles[0], angles[1]); //elbow already at target, move shoulder
        else{
          double[] xy = getXY();
          if (Math.abs(xy[0]) < robotLimit.robotLength/2) run(shldrAngl, angles[1]); //only move the elbow if the arm tip has not cleared the robot,
          else if (xy[1] < -ArmConstants.shoulderHeight + 0.1) run(shldrAngl*0.99, angles[1]); //move the elbow and raise the shoulder a little if the arm tip is at the ground
          else run(angles[0], angles[1]); //move both arms if the arm tip has cleared the robot and not on the ground.
        }
        return true;
      }else return false;
    }else return false;
  ...
  public void moveTowardNeutral(){
    double[] xy = getXY();
    if(xy[1] > robotLimit.height)run(shldrAngl, 0); //if the arm tip is above the limit, lower the elbow only
    else{
      if (Math.abs(shldrAngl) < 0.05)run(0,0); //if the shoulder is at 0, lower the elbow to 0
  ...
  /**
   * Move the arm tip to the desired (x,y) coordinates.
   * use moveTOXYFromNeutral if the starting position is close to neutral since that method has less calculations.
   * @param _x
   * @param _y
   * @return true if a solution is available, otherwise false.
   */
  public boolean moveTowardXY(double _x, double _y){
    if(isXYInLimit(_x, _y)){
      ...
      if(angles[2] > 0) { // a solution is found
        double[] xy = getXY();
        if (xy[0] * _x > 0) moveTowardXYFromNeutral(_x, _y);  //the current and the desired arm tip positions at the same side of the robot
        else {
          if (Math.abs(shldrAngl) < 0.05)run(0, angles[1]); //optimization to avoid the elbow slow down at the neutral position 
          else moveTowardNeutral();  //move to neutral position if optimization cannot be safely performed.
        }
        return true;
      }else return false;}
    return false;
