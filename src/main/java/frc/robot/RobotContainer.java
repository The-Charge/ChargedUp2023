...
public class RobotContainer {
  ...
  private RobotContainer() {
    ...
    SequentialCommandGroup scoreHighCone = new SequentialCommandGroup(
        new ResetHeading(m_drivetrain),
        new ResetPitch(m_drivetrain),
        //new MoveMagicArmToXY(m_arm, 1.5, 1.09, 8000),
        new ScoreHighCone(m_arm, true),
        new OpenClaw(m_claw, m_arm),
        new MoveMagicArmToXY(m_arm, -0.91, 1.65, 8000),
    ...
    SequentialCommandGroup scoreHighBall = new SequentialCommandGroup(
      new ResetHeading(m_drivetrain),
        new ResetPitch(m_drivetrain),
       // new MoveMagicArmToXY(m_arm, 1.54, 1.09, 8000),
       new ScoreHighCone(m_arm, true),
        new OpenClaw(m_claw, m_arm),
        new MoveMagicArmToXY(m_arm, -0.91, 1.65, 8000),
        new CloseClaw(m_claw),
  ...
  private void pathSendableChooserInit() {
    ...
    String[] test = new String[(int)Constants.SysIDConstants.NUMBER_OF_PATHWAYS + 1];
  ...
  private void configureButtonBindings() {
    ...
    frontLowButton.onTrue(
        (new MoveMagicArmToXY(m_arm, 1.0, -0.1, 5000)).withInterruptBehavior(InterruptionBehavior.kCancelSelf));
    JoystickButton frontHighButton = new JoystickButton(armController, 4);
    frontHighButton
        .onTrue((new MoveMagicArmToXY(m_arm, 1.15, .79, 5000)).withInterruptBehavior(InterruptionBehavior.kCancelSelf));
