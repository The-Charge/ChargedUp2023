...
public class MoveArmSimple extends CommandBase {
  ...
  @Override
  public void execute() {
    double xSpeed = -RobotContainer.getInstance().getArmJoystick().getRawAxis(1) / 250; //.getleftJoystick().getY() / 250;
    double ySpeed = -RobotContainer.getInstance().getArmJoystick().getRawAxis(3) / 250; //.getrightJoystick().getY() / 250;
