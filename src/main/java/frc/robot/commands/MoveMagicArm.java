...
public class MoveMagicArm extends CommandBase {
  ...
  // 0: neutral; 1: 1st quad pickup; 2: 1st quad score; 3: 2nd quad pickup; 4: 2nd
  // quad score
    ...
    if (inTransittion || secondStepNeeded) {
      ...
      if (elbowHitIntermediate) {
        ...
        if (armState == 1 || armState == 2)
          ...
          targetElbowAngle = -Math.PI / 2;
        else if (m_arm.getShoulderAngle() < 0)
          targetElbowAngle = Math.PI / 2;
        else
          targetElbowAngle = -Math.PI / 2;
      ...
      if (RobotContainer.getInstance().getleftJoystick().getRawButton(2) && !(armState == 0)) {
        inTransittion = true;
        elbowHitIntermediate = false;
        armState = 0;
      } else if (RobotContainer.getInstance().getleftJoystick().getRawButton(3) && !(armState == 1)) {
        inTransittion = true;
        elbowHitIntermediate = false;
        if (armState == 3 || armState == 4) {
          ...
          armState = 1;
      } else if (RobotContainer.getInstance().getleftJoystick().getRawButton(5) && !(armState == 2)) {
        inTransittion = true;
        elbowHitIntermediate = false;
        if (armState == 3 || armState == 4) {
          secondStepNeeded = true;
